                    ...
                    <div>
                        <div class="d-lg-flex">
                            ...
                            <div class="mb-3 ml-lg-auto">
                                ...
                                <vs-button @click="bulkDelete" color="danger" v-if="bulkDeleteData.length > 0"
                                           type="relief">Hapus Data Terpilih
                                </vs-button>
                            </div>
                        </div>
                        <!--//dt-->
                        ...
                        <div id="print" class="d-none d-print-block">
                            <h4 class="text-center text-capitalize">Report Slider</h4>
                        ...
                        <b-modal :title="modalTitle" v-model="showMyModal" size="xl" hide-footer>
                            ...
                            <b-form-group class="mb-3">
                                <radio-ajax v-model="data.status" url="/api/select/boolean"
                                            :errors="errors.status != undefined ? errors.status[0] : ''"
                                            label="Status Slider"
                                            option-text="text"
                            ...
                            <b-form-group class="mb-3">
                                <radio-ajax v-model="data.publish" url="/api/select/boolean"
                                            :errors="errors.publish != undefined ? errors.publish[0] : ''"
                                            label="Publish"
                                            option-text="text"
                                            option-value="value"></radio-ajax>
                            </b-form-group>
                            <b-form-group class="mb-3">
                                <checkbox-ajax v-model="data.tag" url="/api/select/boolean"
                                               :errors="errors.tag != undefined ? errors.tag[0] : ''"
                                               label="Tag"
                                               option-text="text"
...
export default {
    ...
    methods: {
        async generatePdf() {
            let res = await this.axios.post('/api/slider/api');
            this.dataPrint = res;
            this.$nextTick(() => {
        ...
        update() {
            ...
            let url = `/api/slider/${this.data.id}`;
            this.handleRequest(url, data);
        },
        ...
        bulkDelete() {
            ...
            this.$vs.dialog({
                ...
                accept: function () {
                    this.axios.post('/api/slider/bulkdelete', {data: $vm.bulkDeleteData}).then(res => {
                        if (res.data.status) {
                            $vm.bulkDeleteData = [];
                            this.$vs.notify({
                                title: res.data.text,
